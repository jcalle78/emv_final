...
class Parada extends Serializable
{
  ...
  Future<List> obtenerDatos(int id) async {
    ...
    final String sql = "select * from public.te_parada where rec_id=$id and par_estado=1 order by par_orden";
    final List datos=[];
   ...
   Future<List> obtenerParadaRuta(int id) async {
    final conexion = Conexion();
    final String sql = "select * from public.te_parada p, public.te_ruta u,public.te_recorrido r where r.rut_id=u.rut_id and r.rec_id=p.rec_id and r.rut_id=$id order by 3";
    final List datos=[];
    final List<dynamic> query = await conexion.obtenerTabla(sql);
    print(sql);
    if(query != null && query.isNotEmpty)
  ...
  Future<List> obtenerParadaFuncionario(int id) async {
    final conexion = Conexion();
    final String sql = "select * from public.te_parada p, public.te_ruta u,public.te_recorrido r, public.te_ruta_vehiculo t,public.te_vehiculo v"
                      " where r.rut_id=u.rut_id and r.rec_id=p.rec_id and u.rut_id=t.rut_id and t.veh_id=v.veh_id and v.fun_id=$id";
    final List datos=[];
    final List<dynamic> query = await conexion.obtenerTabla(sql);
